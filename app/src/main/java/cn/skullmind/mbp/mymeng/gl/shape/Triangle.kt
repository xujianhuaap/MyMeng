...
class Triangle {
    ...
    private var vertexBuffer = ByteBuffer.allocateDirect(triangleCoords.size).run {
    ...
    fun loadShader(type:Int, shaderCode:String):Int = GLES20.glCreateShader(type).also {
            shader ->
    ...
    fun draw(){
        ...
        positionHandle = GLES20.glGetAttribLocation(program,"vPosition").also {
            ...
            GLES20.glVertexAttribPointer(it, COORDS_PER_VERTEX,GLES20.GL_FLOAT,
            false,vertexStride,vertexBuffer)

            colorHandle = GLES20.glGetUniformLocation(program,"vColor").also {
                colorHandleParameter ->
    ...
    companion object{
        ...
        var triangleCoords = floatArrayOf(0f,0.65f,0.0f,
        -0.5f,-0.3f,0.0f,
        0.5f,-0.3f,0.0f)
    }
}